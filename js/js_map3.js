	...
	if (firstRun == false) {
		...
		for (var i = 1; i < data.length; i++) {
			if (data[i] != "") {
				...
				if (playerVehicleType == "PLANE" || playerVehicleType == "BOAT" || playerVehicleType == "CAR")
				{
				var playercursor = "vehicle.png";
				}
				else if (playerVehicleType == "HELICOPTER")
				{
					var playercursor = "HeliTempmemes2016.png";
				}
...
//fix playerlisttoggle info text
				...
				if (playercursor == "cursor.png" || playercursor == "death.png" || playercursor == "vehicle.png")
				{
					document.getElementById(playerCSteamID+"_cursor").style.width = 14+"px";
					document.getElementById(playerCSteamID+"_cursor").style.height = 20+"px";
				}
				else if (playercursor == "HeliTempmemes2016.png")
				{
					document.getElementById(playerCSteamID+"_cursor").style.width = 29+"px";
					document.getElementById(playerCSteamID+"_cursor").style.height = 34+"px";
				}
		...
		if (playerListToggle == true && players >= 1)
		{
			$("#playerlistbox").css({'display': 'table'});
			playerlist();
		}
		else
		{
			$("#playerlistbox").css({'display': 'none'});
		}
...
function animate() {
	...
	for (var i = 0; i < animatePlayerCSteamID.length; i++) {
		$("#" + animatePlayerCSteamID[i]).animate({top: animateTop[i], left: animateLeft[i]}, {'transition-duration': syncinterval, 'transition-timing-function': 'linear', queue: false});
